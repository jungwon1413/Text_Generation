...
class novel_dataset:
	...
	def __init__(self, full_filename, seq_len):
		...
		print("Word Slicing...")
		twitter = Twitter()
		text = twitter.morphs(f)	# word split
		text = [' ' if word is "SPACE" else word for word in text]
		text = ['\n' if word is "ENTER" else word for word in text]

		words = sorted(list(set(text)))

		self.word_indices = dict((c, i) for i, c in enumerate(words))
		self.indices_word = dict((i, c) for i, c in enumerate(words))

		idx_text = []
		for i, word in enumerate(text):
			idx_text.append(self.word_indices[word])
		text = idx_text
...
if __name__ == "__main__":
	filename = "D:/Seed_Downloads/Novel Dataset/Asian fantasy/NOVEL_01025.txt"
	seq_len = 26
	novel = novel_dataset(filename, seq_len)

	data_dim = len(novel.indices_word)
	hidden_size = len(novel.indices_word)
	num_classes = len(novel.indices_word)
	learning_rate = 0.001

	print("Word set length: %s" % data_dim)	# length check
	...
	for i in range(500):
		...
		if i % 10 is 0:
			print(i, j, ''.join([novel.indices_word[t] for t in index]), l)